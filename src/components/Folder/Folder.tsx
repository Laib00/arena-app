...
export default function Folder({
  ...
  const handlePaperClick = (e: MouseEvent<HTMLDivElement>, index: number) => {
    e.stopPropagation();
    if (!open) return;
    onItemClick?.(index);
  };
  ...
  const folderStyle: CustomProperties = {
    "--folder-color": color,
          ...
          {papers.map((item, i) => (
            <div
              ...
              className={[
                "paper",
                `paper-${i + 1}`,
                onItemClick ? "paper--interactive" : "",
                activeIndex === i ? "paper--front" : "",
              ]
                .filter(Boolean)
                .join(" ")}
