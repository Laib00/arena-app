...
import type { AgentProfile, Aim, ChatMessage, Persona, Setting } from "./types/domain";

type EvalSections = Record<string, string>;
...
type DebriefStep = "reflection" | "feedback" | "reflection_update" | "facts" | "suggestions" | "done";

/** Parse OVERALL / STRENGTHS / … blocks from AI suggestions text. */
export function parseEvalSections(text: string | null | undefined): EvalSections | null {
...
const STEP_ORDER: DebriefStep[] = ["reflection", "feedback", "reflection_update", "facts", "suggestions"];

function emptyAnswers(prompts: string[]): string[] {
  return prompts.map(() => "");
}

/** Serialize Q&A for DB (JSON string). Returns null if all blank. */
export function serializeReflectionAnswers(
  prompts: string[],
  answers: string[]
): string | null {
...
export function formatReflectionForPrompt(serialized: string | ReflectionPayload | null | undefined): string {
  if (!serialized) return "(none)";
  try {
    const data = typeof serialized === "string" ? JSON.parse(serialized) : serialized;
    if (!data?.items?.length) return "(none)";
    return data.items
      .map((item: ReflectionItem, i: number) => `Q${i + 1}. ${item.question}\nA: ${item.answer || "(blank)"}`)
      .join("\n\n");
  } catch {
...
export function ReflectionAnswersView({ value }: { value: string | ReflectionPayload | null | undefined }) {
  if (!value) {
  ...
  try {
    ...
    if (Array.isArray(data?.items)) items = data.items as ReflectionItem[];
  } catch {
    /* legacy plain string */
  }
...
function StepDots({ step }: { step: DebriefStep }) {
...
export default function SessionDebrief({
  ...
  callAI: (systemPrompt: string, messages: ChatMessage[]) => Promise<string>;
  onSaveDebrief: (payload: Record<string, unknown>) => Promise<void> | void;
}) {
  const [step, setStep] = useState<DebriefStep>("reflection");
  ...
  const [suggestionsSections, setSuggestionsSections] = useState<EvalSections | null>(null);
  ...
  async function generateFeedback() {
    ...
    try {
      ...
      setError("Couldn't generate client feedback. " + (e instanceof Error ? e.message : String(e)));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function generateFacts() {
    ...
    try {
      ...
      setError("Couldn't generate facts. " + (e instanceof Error ? e.message : String(e)));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function generateSuggestions() {
    ...
    try {
      ...
      setError("Couldn't generate AI suggestions. " + (e instanceof Error ? e.message : String(e)));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function finish({ includeSuggestions = false } = {}) {
    ...
    try {
      ...
      setError("Couldn't save this session debrief. " + (e instanceof Error ? e.message : String(e)));
    } finally {
      setSaving(false);
    }
  }
