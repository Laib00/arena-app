...
import type { Aim, Challenge, Grade, Industry, Persona, Setting } from "../types/domain";
...
const FIELD_LABELS: Record<Industry, [string, string, string]> = {
...
function toPersonaObj(row: (string | number)[], industry: Industry): Persona {
  ...
  return {
    ...
    edu: String(edu),
    disc: disc as Persona["disc"],
    badExp: String(badExp),
    badExpReason: String(badExpReason),
    needLevel: String(needLevel),
...
const GRADE_ORDER: Grade[] = ["Easy", "Medium", "Hard", "Impossible"];
...
function randomInvestable(): string {
...
function generateRandomClient(industryKey: Industry): Persona {
  // industryKey: "Property" or "FP"
  const grade = pick(["Easy", "Easy", "Medium", "Medium", "Medium", "Hard", "Hard", "Impossible"] as const);
  const disc = pick(["D", "I", "S", "C"] as const);
  const badExp = grade === "Easy" ? "No" : pick(["Yes", "Yes", "No"] as const);
  const badExpReason =
    badExp === "Yes"
